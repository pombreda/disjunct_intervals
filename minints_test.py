...
def test_mixed_intervals_2():
    ints = (
        [23, 45], [1, 3], [5, 9], [13, 19],
        [41, 49], [14, 15], [17, 18], [19, 19]
    )
    solution = minints.gen_disjunct_intervals(ints)
    assert len(filter(lambda x: x != [], solution.values())) == 4
    for item in solution.values():
        if len(item) > 1:
            if len(item) > 2:
                assert [13, 19] in item
                ...
                assert [23, 45] in item
                assert [41, 49] in item
        elif len(item) == 1:
            assert [1, 3] in item or [5, 9] in item
        else:
            assert item == []
...
def test_equal_intervals_with_dates():
        import datetime
        ints = [
            [datetime.date(2007, 5, 7), datetime.date(2013, 11, 5)],
            [datetime.date(2007, 5, 7), datetime.date(2013, 11, 5)],
        ]
        solution = minints.gen_disjunct_intervals(ints)
        for item in solution.values():
...
def test_disjunct_intervals_ITER():
    ints = ([1, 3], [5, 6], [13, 16])
    solution = minints.gen_disjunct_intervals(ints, 'ITER')
    for item in solution.values():
        assert len(item) == 1
        assert item[0] in ints


def test_touching_intervals_ITER():
    ints = ([2, 6], [6, 8], [11, 14])
    solution = minints.gen_disjunct_intervals(ints, 'ITER')
    for item in solution.values():
        if len(item) > 1:
...
def test_overlapping_intervals_ITER():
    ints = ([2, 6], [4, 8], [11, 14])
    solution = minints.gen_disjunct_intervals(ints, 'ITER')
    for item in solution.values():
        if len(item) > 1:
...
def test_mixed_intervals_1_ITER():
    ints = ([1, 3], [5, 9], [13, 19], [14, 15], [17, 18], [19, 19])
    solution = minints.gen_disjunct_intervals(ints, 'ITER')
    assert len(filter(lambda x: x != [], solution.values())) == 3
    for item in solution.values():
...
def test_mixed_intervals_2_ITER():
    ints = (
        [23, 45], [1, 3], [5, 9], [13, 19], [41, 49], [14, 15], [17, 18],
        [19, 19]
    )
    solution = minints.gen_disjunct_intervals(ints, 'ITER')
    assert len(filter(lambda x: x != [], solution.values())) == 4
...
def test_equal_intervals_with_dates_ITER():
        import datetime
        ints = [
            [datetime.date(2007, 5, 7), datetime.date(2013, 11, 5)],
            [datetime.date(2007, 5, 7), datetime.date(2013, 11, 5)],
